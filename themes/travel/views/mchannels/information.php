...
<div class="w750">
   <div class="news_search"><!--搜索-->
       ...
       <table>
          <tr>
            <td><strong>标题：</strong><input name="title" value="<?php echo $page_params['title']; ?>" type="text" /></td>
            <td><strong>资讯类别：</strong>
            	<?php
            	  $channel_category=ChannelCategory::model();
            	  $category_select=$channel_category->get_select("53");
            	  $category_select=Util::com_search_item(array(''=>'分类'),$category_select);
            	  echo EHtml::createSelect("category",$page_params['category'],$category_select,array());
            	?>
            
            
            </td>
            <td><input type="submit" value="搜索" class="ss_bnt"/></td>
          </tr>
       </table>
       <?php $this->endWidget(); ?>
   </div><!--//搜索-->
