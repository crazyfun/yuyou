...
class ShowAction extends BaseAction{
   ...
   protected function do_action(){
    ...
    $archive_data=$archives_model->with("Channels","ChannelCategory","EndRegion","StartRegion")->findByPk($archive);
    ...
    if(Util::view_permission($permission)){
    	...
    	$travel_tags=$archive_data->travel_tags;
    	 $ip_convert=IpConvert::get();
		    $region_data=$ip_convert->init_region();
   		WebConfig::set_seo_content(array('seo_title'=>$region_data['name']."出发".$seo_title.",".$travel_tags),array('seo_keywords'=>$region_data['name']."出发".$seo_keywords.",".$travel_tags),array('seo_description'=>$region_data['name']."出发".$seo_describe.",".$travel_tags));
