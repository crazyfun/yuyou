   	    ...
   	    if($action=="order"){
   	     if($people_nums==$contacter_number){
   	    	if($_POST[$main_model_name]){
		    		...
		    		if($_POST[$model_name]){
							...
							if($redirect_flag){
								...
								$this->controller->redirect($this->controller->createUrl("pay/step3",array('order_id'=>$order_id)));
							}else{
								$this->controller->set_flash("请填写正确的游客信息",CV::FAIL);
							}
						}else{
							$this->controller->set_flash("请填写游客信息",CV::FAIL);
						}
				  }
				 }else{
				 	  $this->controller->set_flash("请填写联系人信息",CV::FAIL);
				 }
				}else{
					$this->controller->set_flash("游客信息数量不正确",CV::FAIL);
				}
   	    }else{
   	    	$model->order_id=$order_id;
   	    	$main_model->order_id=$order_id;
   	    	if(!Yii::app()->user->isGuest){
   	    		...
   	    		$main_model->contacter=$user_data->real_name;
   	    		$main_model->contacter_phone=$user_data->cell_phone;
   	    	}	    	
   	    }
