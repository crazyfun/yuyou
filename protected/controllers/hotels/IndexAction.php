    ...
    if(Util::view_permission($permission)){
      $name=$channels_data->name;
    	...
    	$cover_template=Util::get_file_name($cover_template);
			$this->display($cover_template,array('cssPath'=>$cssPath,'jsPath'=>$jsPath,'name'=>$name,'content'=>$channels_data->content,'channel'=>$channel,'category'=>$category,'channel_parent'=>$parent_id,'pattern'=>$channels_data->pattern));
    }
   }
