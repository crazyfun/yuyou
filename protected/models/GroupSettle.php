 ...
 */
class GroupSettle extends BaseActive
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('group_id,type,out_serial,status','required','message'=>'{attribute}不能为空'),
			array('type, status', 'numerical', 'integerOnly'=>true),
			array('group_id, create_time', 'length', 'max'=>11),
			array('out_serial', 'length', 'max'=>100),
			array('comment','safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id,comment, group_id, type, out_serial, status, create_time', 'safe', 'on'=>'search'),
		);
	}
	...
	function show_attribute($attribute_name){
		switch($attribute_name){
		    ...
		    case 'status':
		    	...
		    	return empty($this->status)?"未结算":$group_settle[$this->status];
		    	break;
		    case 'create_time':
