 ...
 */
class Gallery extends BaseActive
{
	/**
  ...
  public $create_name="";
	...
	public function rules()
	{
		...
		return array(
			...
			array('stitle, image,gallery_tags,attr', 'length', 'max'=>100),
	...
	public function search()
	{
		...
		$criteria->compare('gallery_tags',$this->gallery_tags,true);
	...
	public function delete_table_datas($pk_id="",$condition=array()){
		...
		$gallery_images=GalleryImages::model();
		if(is_array($table_datas)){
		foreach($table_datas as $key => $value){
			$id=$value->id;
			$channel_id=$value->channel_id;
			$this->deleteByPk($id,"",array());
			$save_path="upload/gallery/".$channel_id;
			Util::deleteDir($save_path);
			$gallery_images->delete_table_datas("",array('condition'=>'t.gallery_id=:gallery_id','params'=>array(':gallery_id'=>$id)));
		}
	 }else{
	 	  $id=$table_datas->id;
	 	  $channel_id=$value->channel_id;
	...
	function show_attribute($attribute_id){
	 switch($attribute_id){
      ...
      $gallery_att_select=$config_values->findAll("FIND_IN_SET(value,'".$this->attr."')>0 AND type=:type",array(':type'=>'1'));
      $att_name=array();
      foreach($gallery_att_select as $key => $value){
	    ...
	    case 'create_id':
	      ...
	      break;
	    case 'status':
	      $gallerys_status=CV::$archives_status;
	      return $gallerys_status[$this->status];
	      break;
	...
	function show_channel($id){
		$gallery_data=$this->with("Channels")->findByPk($id);
		return $gallery_data->Channels->name;
	}
	
	function show_category($id){
		$gallery_data=$this->with("ChannelCategory")->findByPk($id);
		return $gallery_data->ChannelCategory->name;  
	}
	
	function show_create_id($id){
		$gallery_data=$this->with("User")->findByPk($id);
		return $gallery_data->User->user_login;
	}
	...
	function get_operate(){
		  ...
		  $return_str.=CHtml::link('预览',"javascript:self.parent.parent.location.href='".Yii::app()->homeUrl."/gallery/show/id/".$this->id."'",array('class'=>'operate_green','target'=>'_blank'));
		  if(Util::is_permission($user_permission_name,"status"))
	...
	function show_galley_images($gallery_id=""){
		 $gallery_id=empty($gallery_id)?$this->id:$gallery_id;
		 $gallery_images=GalleryImages::model();
		 $gallery_images_number=$gallery_images->count("t.gallery_id=:gallery_id",array(':gallery_id'=>$gallery_id));
		 $return_array=CHtml::link("查看或增加图片(".$gallery_images_number.")",'javascript:void(0);',array('class'=>'operate_green','onclick'=>"javascript:frame_gallery('".Yii::app()->getController()->createUrl('galleryimages/index')."','下载列表','".$gallery_id."');"));
		 return $return_array;
	}
	...
	function set_channel_link($pattern,$id){
		switch($pattern){
        		case 'archives':
    	      	return YYii::app()->getController()->createUrl("mchannels/show",array('id'=>$id));	
        		  break;
        		case 'travel':
