 ...
 */
class GroupPays extends BaseActive
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'GroupOrder'=>array(self::BELONGS_TO,'GroupOrder','order_id'),
		);
	}
	...
	function show_attribute($attribute_name){
		switch($attribute_name){
			case 'order_id':
				...
				break;
			case 'group_id':
			  return $this->GroupOrder->Group->title;
			  break;
			case 'user_id':
	...
	function change_order_status($order_id,$outer_serial=""){
       ...
       try {
              ...
              if($status=='1'){
              	  ...
              	  if($result){
              	  	$group_order_id=$pays_data->order_id;
              		 	$group_order=GroupOrder::model();
              			$group_order_data=$group_order->with("Group")->findByPk($group_order_id);
              		  $group_order->updateByPk($group_order_id,array('pay_status'=>'2','pay_time'=>time()));
              		  $send_message=new SendMessage();
   	    						$send_message->send_message(22,$group_order_data->cell_phone,array('order_serial'=>$group_order_data->order_serial,'title'=>$group_order_data->Group->title,'contact_phone'=>$group_order_data->Group->Company->company_name,'address'=>$group_order_data->Group->Company->address));
              	}
             }
             $transaction->commit();      
       }catch(Exception $e){
