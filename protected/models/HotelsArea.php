...
/**
 * This is the model class for table "{{hotels_area}}".
 *
 * The followings are the available columns in table '{{hotels_area}}':
 * @property string $id
 * @property string $hotels_id
 * @property string $area_name
 * @property integer $sort_order
 */
class HotelsArea extends BaseActive
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return HotelsArea the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('hotels_id,area_name', 'required','message'=>'{attribute}不能为空'),
			array('sort_order', 'numerical', 'integerOnly'=>true),
			array('hotels_id', 'length', 'max'=>11),
			array('area_name', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, hotels_id, area_name, sort_order', 'safe', 'on'=>'search'),
		);
	}
