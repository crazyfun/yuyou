...
/**
 * This is the model class for table "{{travel_flash_ad}}".
 *
 * The followings are the available columns in table '{{travel_flash_ad}}':
 * @property string $id
 ...
 */
class TravelFlashAd extends BaseActive
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return TravelFlashAd the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('flash_img,img_href,region_ids,sort', 'required','message'=>'{attribute}不能为空'),
			array('sort', 'numerical', 'integerOnly'=>true),
			array('title','length','max'=>'30'),
			array('region_ids, flash_img, img_href', 'length', 'max'=>100),
			array('describe', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, title,region_ids, flash_img, img_href, describe, sort', 'safe', 'on'=>'search'),
		);
	}
	...
	function get_content($region_id){
	  $contents=$this->findAll(array('condition'=>"FIND_IN_SET('".$region_id."',t.region_ids)>0",'params'=>array(),'order'=>'sort ASC'));
