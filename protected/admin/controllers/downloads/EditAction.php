...
class EditAction extends BaseAction{
  ...
  protected function do_action(){ 
  	...
  	if(isset($_POST[$model_name])){
		  ...
		  $downloads_att=$_REQUEST['downloads_att'];
		  $select_image=$_REQUEST['select_image'];
		  $category_id=$_REQUEST['category_id'];
		  $model->category_id=$category_id;
		  $model->attr=implode(",",(array)$downloads_att);
		  if(empty($addition_select)){
		  	$addition_select=array();
		  }
		  $save_path="upload/downloads/".$model->channel_id;
		  if(in_array('1',$addition_select)){
		   ...
		   if(!$select_image){	
			...
			if($model->validate()){			 
			  ...
			  if($result){
			  	 if(!empty($model->downloads_tags)){
			  	 	  $insert_downloads_tags=explode(",",$model->downloads_tags);
			  	 	  $archive_tags=ArchiveTags::model();
			  	 	  foreach($insert_downloads_tags as $key => $value){
		  ...
		  $attr=$model->attr;
		  $downloads_att=explode(",",$attr);
		  $addition_select=array('1','2');
		  
	 }
	 $this->display('add',array('model'=>$model,'addition_select'=>$addition_select,'downloads_att'=>$downloads_att));
  } 
