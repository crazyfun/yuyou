  	...
  	if(isset($_REQUEST['action'])){
       ...
       if($model->validate()){
           ...
           $this->controller->f(CV::FAILED_ADMIN_OPERATE);
			...
			$alipay=$model->find('pay_code=:pay_code AND t.company_id=:company_id',array(':pay_code'=>'alipay',':company_id'=>$this->controller->company_id));
			$alipay_config=array();
			if(empty($alipay)){
			...
			$kuaiqian=$model->find('pay_code=:pay_code AND t.company_id=:company_id',array(':pay_code'=>'kuaiqian',':company_id'=>$this->controller->company_id));
