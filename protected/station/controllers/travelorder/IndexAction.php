...
class IndexAction extends BaseAction{
  ...
  protected function do_action(){
   ...
   $model=TravelOrder::model();
	 ...
	 $page_params=array();
	  ...
	  $reserved=$_REQUEST['reserved'];
		...
		if(!empty($relation_operate_id)){
			 array_push($conditions,"t.operate_id = :relation_operate_id");
    ...
    array_push($conditions,"Travel.company_id = :travel_company_id");
    ...
    if(Util::is_permission($user_permission_name,"setuser")){
    	...
    	array_push($conditions,"t.operate_id = :operate_id");
	 ...
	 $dataProvider=new CActiveDataProvider($model, array(
		'criteria'=>array(
			...
			'params'=>$params,
			'with'=>array("Travel","User","Company",'TravelOrderContacter'),
			'together'=>true,
			'group'=>'t.id',
		),
		'pagination'=>array(
